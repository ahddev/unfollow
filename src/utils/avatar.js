...
export async function fetchInstagramAvatar(username) {
      ...
      if (response.ok) {
        ...
        if (typeof html === 'string') {
          ...
          if (metaMatch && metaMatch[1]) {
            ...
            // Use smaller size for faster loading (replace with smaller size parameter if available)
          ...
          if (jsonLdMatch) {
            try {
              ...
              if (jsonData.image || jsonData?.author?.image) {
                const imgUrl = jsonData.image || jsonData.author.image;
  ...
  // Method 2: Return a placeholder pattern (component will show default icon)
  // We don't return a fallback URL to avoid broken images
  return null;
}
